...
This version implements parent preparation and the complete fourteen-chord color/animal phase. Individual-note training and expert transition criteria remain deferred. The server now reviews synced practice every fourteen local calendar days using GPT-6 Astra with medium reasoning and may introduce exactly one new white-key chord. Parents retain manual plan controls and can pause AI reviews. See [AI review operation and evaluation](ai-reviews.md). Historical adaptive-learning modules remain in the repository for reference; they are not a prescription for the new program.

Practice interaction: after preparation, the home screen starts practice directly with “Let’s listen”. The optional sound-check route retains Ready and replay controls. Tapping an animal shows the correct color/animal, then advances after the configured feedback interval (three seconds by default) and completion of the current audio. This feedback interval is an app interaction choice, not an official progression criterion. Book 2 Q7 (pp. 33–35 in the supplied translation) prescribes consistent chord duration; Q26 (pp. 85–86) describes promptly giving a hesitant child the answer and moving on. Replay, pause, early finish, and a small no-response control remain available; there is no answer timeout. Ready-screen previews mark the upcoming session as having a recent pitch reference. Parent assistance on feedback is an immutable, reversible annotation on the original tap; record summaries exclude assisted answers from unaided recognition and confusion pairs.
...
### Restored playroom design: book check, September 12, 2026

The pre-foundation interface supplied the visual reference: `6749617` for Iris's pink (`#FFD6E7`), `8b373d6` for the six-color picker, `4f982de^` for the side-by-side animal layout, and `2c91891^` for the automatic-transition progress indicator. These presentation choices are restored without reinstating the historical mastery, octave, or cue-fading rules.

- **Visible, stable colors:** Book 3 Q24 (source pp. 87–90) permits other associations but cautions that a favorite character can bias answers. Q25 (pp. 91–94) emphasizes seeing the color and physically raising a flag. The cards now have large fields in their assigned cue colors; names, order and pitches remain unchanged. Pictures and tapping remain app adaptations, not evidence of equivalence to raising flags.
- **Warmth without grading:** Book 3 Q69 (pp. 226–228), checked against the Japanese as well as the supplied AI translation, says not to praise or scold according to performance. The UI therefore gives the correct cue the same brief, gentle motion after an independent, incorrect, or no-response turn. No sad/disappointed expressions, correctness-dependent celebration, pitched reward sound, or idle animation while awaiting an answer. Reduced Motion disables the hop. Applying the book's advice to animation is a design interpretation; the book does not address digital animal animations directly.
- **Predictable transition:** The bar appears only during the feedback interval (three seconds by default, adjustable from one to ten seconds in Parent settings). Pause freezes it; resuming or recording parent help restarts the interval. Saving failures prevent advancement, and any still-playing chord finishes before the next presentation. It is not an answer deadline. Book 2 Q7 and Q26 provide the duration/help principles described above; the exact UI timer remains an app choice.
- **Child-chosen setting:** Pink, Cloud, Sky, Lavender, Mint and Sunshine are available from the palette button before practice and Parent settings → Background. The choice is static across sounds, never a hint. It is saved per account on the device, including a separate debug preference, and reads the previous playroom preference on first use.

Nine opaque white-backed illustrations received background-only edits through the built-in image tool, saved as `*-v2.png`; the five existing transparent illustrations retain their assets. The final images use colored backgrounds because the first transparency attempt produced an opaque checkerboard and was rejected. Prompt template: “Minimal edit for an existing children's app. Preserve the supplied [animal] exactly: same species, fur/feather/body colors, expression, anatomy, placement, size, outlines and illustration style. Change ONLY the white/off-white background to a completely flat, uniform solid [cue hex] color, all the way to all four edges. No gradient, texture, checkerboard, transparency, shadow, text, or accessories. Preserve light-colored details inside the animal. Square image, same composition and margins.” All nine outputs were visually inspected. Exact hex values are UI choices, not colorimetric specifications from the books.
...
September 12 restoration validation: 266 frontend tests in 41 suites passed, along with TypeScript, Expo lint, formatting of changed TS/TSX files, and diff whitespace checks. Rendered web checks at 393×852 and 1024×768 covered two and fourteen animals, the saved background after reload, feedback, parent-help annotation, pause/resume and early finish; the browser error log was empty. An iPad (A16) simulator with iOS 26.5 was created and a standalone Release app installed. Parent preparation, the fourteen-animal plan, portrait (four columns) and landscape (seven columns) practice, feedback bar, no-response recording, pause/resume and saving two responses were verified. One initial piano-loading stall after setup resolved by reopening practice; its cause was not established. These are simulator checks, not physical-device audio-quality or notification-delivery proof.

### Shorter daily flow, September 12, 2026

After preparation, home shows the current animals, a start button and today’s remaining sessions. The single Parents control opens a settings overview containing AI reviews and animal plans, routine/rest days, reminders, records, background, account/email, sound check and the guide. Home no longer asks for a manual check-in or sharing. The actual server AI review date is labeled upcoming or pending in the review controls; the optional parent-note schedule is not substituted for it. Notes can be added at any time, and export remains an optional record action.

Direct start retains the brief recent-music question on home and passes its explicit answer into the session event. The initial answer is unknown and resets on returning home; omitting it never creates clean first-sound evidence. Sound-check previews still mark the session as referenced. Preparation, rest, daily limits, spacing, audio readiness and successful-playback gates remain enforced by practice itself. No new transition criteria or backend scheduling changes are introduced.

### Parent practice controls

Parent settings → Practice controls adjusts the feedback bar from one to ten seconds, enables or disables animal motion, and chooses a hold or tap for no response. Three seconds, motion on, and a one-second hold are the defaults. System Reduced Motion still overrides animation. The feedback interval remains an app interaction choice, not a change to the fixed three-second chord stimulus or progression rules.

No response is visually secondary. Ordinary touch/mouse taps do nothing in hold mode; a one-second press records the existing no-response outcome. Keyboard and assistive activation remain accessible, and the control is disabled while paused or saving. These local options share the existing account/device playroom storage and remain separate in debug mode. Saving a palette choice preserves the timing and gesture options. Practice waits for saved controls to load before beginning.
