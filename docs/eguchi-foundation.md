...
Native notification permission is requested only when a parent enables a device reminder. Device reminders currently use the local scheduler. Remote delivery is implemented but disabled until app-specific push credentials and provisioning are ready: set app `extra.remotePushEnabled=true` and backend `EGUCHI_REMOTE_PUSH_ENABLED=true` only after configuring APNs/FCM and confirming delivery. The existing ad hoc profile has no APNs entitlement, so the optional config plugin removes that entitlement in local-only builds. Registered physical devices can then use the backend push worker. Guests and devices without remote registration use seven days of local practice reminders plus the future review reminder, within the iOS pending-notification limit. Opening the app refreshes that rolling schedule and cancels reminders for completed or paused days. Web users can enable email after signing in. Device notifications are not offered as a web capability.
...
Animal art lives in `frontend/assets/images/eguchi/foundation/`. Ten existing characters were edited with the built-in image-generation tool to match their learning cues: red fox, black turtle, green frog, pink bunny, brown lion, light-green parrot, peach crab, lavender bird, gray seal, and light-blue fish. Yellow chick, blue whale, orange tiger, and purple octopus retain their original art. White card backgrounds are intentional. The original artwork remains intact.
